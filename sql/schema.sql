-- АвтоЗапчасть Database Schema — Full Version
...
-- --------------------------------------------------------
-- Table: exchange_rates  (валютные курсы)
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS `exchange_rates` (
  ...
  `rate`       DECIMAL(15,6)  NOT NULL DEFAULT 1.000000 COMMENT 'How many units of this currency = 1 RUB',
  `is_active`  TINYINT(1)     NOT NULL DEFAULT 1,
  `updated_at` DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
-- --------------------------------------------------------
-- Table: warehouse_cache  (кэш склада Москвы)
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS `warehouse_cache` (
  `id`               INT UNSIGNED   NOT NULL AUTO_INCREMENT,
  `part_number`      VARCHAR(80)    NOT NULL UNIQUE,
  `warehouse_stock`  INT            NOT NULL DEFAULT 0,
  `warehouse_price`  DECIMAL(10,2)  DEFAULT NULL,
  `warehouse_eta`    VARCHAR(40)    DEFAULT NULL COMMENT 'estimated delivery time',
...
('Свечи зажигания',    'svechi-zazhiganiya', 4,    'Свечи зажигания и накаливания', 9, 1),
...
('128501FEBI',   'Термостат BMW/Febi',                    'Термостат для системы охлаждения BMW, Mercedes. Карта температур 80-92°C.',                                                         5, 1, 2100.00, 25,  0.210, '90x70x60',   '[]', 1, 1),
...
('social_telegram',      'https://t.me/avtozapchast'),
('social_whatsapp',      'https://wa.me/78005553535'),
('working_hours',        'Пн–Пт: 9:00–20:00, Сб–Вс: 10:00–18:00'),
('delivery_info',        'Доставка по Москве 24ч, по России 2-5 дней');

-- Exchange rates (base currency: RUB = 1)
INSERT INTO `exchange_rates` (`currency`, `name`, `symbol`, `rate`, `is_active`) VALUES
('RUB', 'Российский рубль',  '₽',    1.000000, 1),
('USD', 'Доллар США',         '$',    0.011000, 1),
('EUR', 'Евро',               '€',    0.010200, 1),
('TJS', 'Таджикский сомони',  'сом.', 0.120000, 1),
('KZT', 'Казахстанский тенге','₸',    5.100000, 1);
